					...
					<c:forEach items="${list }" var="board">
						...
						<div class="item">
							...
							<div class="cont">
								...
								<div class="date">
									<span>
									<fmt:formatDate value="${board.wdate}" pattern="yy년 M월 d일"/>
									</span>
								</div>
							</div>
						</div>
					...
					<c:forEach begin="${paging.startPageNo }"
						...
						<c:choose>
							<c:when test="${status.first }">
								<a href="javascript:pageGo('${paging.boardType }', '${paging.firstPageNo }')" class="num">&lt; ${status.current }</a>
							</c:when>
							<c:when test="${status.current == paging.pageNo }">
								<a href="javascript:pageGo('${paging.boardType }', '${status.current }')" class="num on">${status.current }</a>
							</c:when>
							<c:when test="${status.last }">
								<a href="javascript:pageGo('${paging.boardType }', '${paging.endPageNo }')" class="num">${status.current } &gt;</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:pageGo('${paging.boardType }', '${status.current }')" class="num">${status.current }</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</div>
